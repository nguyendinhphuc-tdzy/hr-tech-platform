...
const CVScanView = () => {
    ...
    const [scanningPhase, setScanningPhase] = useState(0); // 0: IDLE, 1: Extract, 2: Evaluate
                ...
                <div className="card-dark" style={{ 
                    ...
                    {!result ? (
                        ...
                        <div className="analysis-result fade-in">
                            ...
                            {/* 2. AI Analysis Content - Hỗ trợ cả format cũ và deep_analysis mới */}
                            {(() => {
                                const ai = result.ai_analysis || {};
                                const deep = ai.deep_analysis || {};
                                const skills = ai.skills || [];
                                const summary = deep.match_reason_and_insights || ai.summary || ai.match_reason || "Không có phân tích.";
                                const strengths = deep.strengths || [];
                                const weaknesses = deep.weaknesses_and_red_flags || [];
                                const missingSkills = deep.missing_skills || [];
                                const usp = deep.unique_highlight || "";
                                    ...
                                    <div style={{ display: 'grid', gap: '25px' }}>
                                        ...
                                        <div style={{ 
                                            ...
                                            <h3 style={{ margin: '0 0 10px 0', color: 'var(--neon-green)', fontSize: '16px', fontWeight: '600', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                                <i className="fa-solid fa-wand-magic-sparkles"></i> TỔNG QUAN & LẬP LUẬN
                                            </h3>
                                            <p style={{ margin: 0, lineHeight: '1.8', color: 'var(--text-secondary)', fontSize: '14px', whiteSpace: 'pre-line' }}>
                                                {summary}
                                            </p>
                                        </div>
                                            ...
                                            <div>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px' }}>
                                                    {skills.map((skill, idx) => (
                                                        <span key={idx} style={{ 
                                                            background: '#1A2736', color: '#E0E0E0', 
                                                            padding: '6px 14px', borderRadius: '20px', fontSize: '13px', 
                                                            border: '1px solid #2D3B4E', boxShadow: '0 2px 4px rgba(0,0,0,0.2)'
                                                        }}>
                                                            {skill}
                                                        </span>
                                                    ))}
                                            ...
                                            <div style={{ display: 'flex', gap: '15px' }}>
                                                {strengths.length > 0 && (
                                                    <div style={{ flex: 1, background: 'rgba(16, 185, 129, 0.05)', padding: '15px', borderRadius: '8px', borderLeft: '3px solid #10B981' }}>
                                                        <h4 style={{ color: '#10B981', margin: '0 0 10px 0', fontSize: '14px' }}>
                                                            <i className="fa-solid fa-arrow-trend-up"></i> Điểm Mạnh
                                                        </h4>
                                                ...
                                                {weaknesses.length > 0 && (
                                                    <div style={{ flex: 1, background: 'rgba(239, 68, 68, 0.05)', padding: '15px', borderRadius: '8px', borderLeft: '3px solid #EF4444' }}>
                                                        <h4 style={{ color: '#EF4444', margin: '0 0 10px 0', fontSize: '14px' }}>
                                                            <i className="fa-solid fa-triangle-exclamation"></i> Lỗ Hổng / Rủi Ro
                                                        </h4>
                                                        <ul style={{ margin: 0, paddingLeft: '18px', color: 'var(--text-secondary)', fontSize: '13px', lineHeight: '1.8' }}>
                                        ...
                                        {/* USP */}
                                        {usp && (
                                            <div style={{ background: 'rgba(245, 158, 11, 0.05)', padding: '15px', borderRadius: '8px', borderLeft: '3px solid #F59E0B' }}>
                                                <h4 style={{ color: '#F59E0B', margin: '0 0 8px 0', fontSize: '14px' }}>
                                                    <i className="fa-solid fa-star"></i> Điểm Nổi Bật Độc Bản (USP)
                                                </h4>
                                                <p style={{ margin: 0, color: 'var(--text-secondary)', fontSize: '13px', lineHeight: '1.6' }}>{usp}</p>
                                            </div>
                                        )}

                                        {/* MISSING SKILLS */}
                                        {missingSkills.length > 0 && (
                                            <div style={{ background: '#0D1825', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)' }}>
                                                <h4 style={{ color: '#F59E0B', margin: '0 0 10px 0', fontSize: '14px' }}>
                                                    <i className="fa-solid fa-link-slash"></i> Kỹ Năng Đoản Khuyết (Cần Đào Tạo)
                                                </h4>
                                                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                                                    {missingSkills.map((s, i) => (
                                                        <span key={i} style={{ 
                                                            fontSize: '12px', background: '#374151', padding: '5px 10px', 
                                                            borderRadius: '4px', border: '1px solid #4B5563', color: '#D1D5DB' 
                                                        }}>{s}</span>
                                                    ))}
                                                </div>
                                            </div>
