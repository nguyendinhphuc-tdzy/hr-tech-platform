/* FILE: frontend/src/views/CVScanView.jsx (Final: Batch Scan Integrated) */
import { useState, useEffect } from 'react';
import axios from 'axios';
import API_BASE_URL from '../components/config';
import CVUpload from '../components/CVUpload'; // Import Component Upload Mới
    ...
    return (
        <div className="cv-scan-view" style={{color: 'var(--text-primary)', paddingBottom: '50px', minHeight: 'calc(100vh - 100px)'}}>
            ...
            <div style={{maxWidth: '900px', margin: '0 auto'}}>
                ...
                {/* 1. Chọn Job */}
                <div style={{marginBottom: '25px', background: 'var(--bg-secondary)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)'}}>
                    <label style={{display: 'block', marginBottom: '10px', fontSize: '14px', color: 'var(--text-primary)', fontWeight: '600'}}>
                ...
                {(isScanning || scanQueue.length > 0) && (
                    <div style={{marginTop: '25px', background: 'var(--bg-secondary)', padding: '20px', borderRadius: '12px', border: '1px solid var(--border-color)', animation: 'fadeIn 0.3s'}}>
                        <h4 style={{margin: '0 0 15px 0', fontSize: '15px', color: 'var(--text-primary)', display: 'flex', justifyContent: 'space-between'}}>
                        ...
                        <div style={{display: 'flex', flexDirection: 'column', gap: '10px', maxHeight: '200px', overflowY: 'auto'}}>
                            {scanQueue.map((item, idx) => (
                                <div key={idx} style={{
                                    ...
                                    fontSize: '13px', padding: '10px 15px', borderRadius: '8px',
                                    background: item.status === 'processing' ? 'rgba(252, 211, 77, 0.1)' : 'var(--bg-input)',
                                    border: item.status === 'processing' ? '1px solid #FCD34D' : '1px solid transparent'
                                }}>
                                    <span style={{display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-primary)'}}>
            ...
            {scannedResults.length > 0 && (
                <div style={{marginTop: '50px'}}>
                    <h3 style={{
                        ...
                        color: 'var(--text-primary)', fontSize: '18px', display: 'flex', alignItems: 'center', gap: '10px'
                    }}>
                    ...
                    <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '25px'}}>
                        {scannedResults.map(candidate => (
                            <CandidateCard 
                                ...
                                // Không cần onStatusChange ở đây vì chỉ là view kết quả tĩnh
                            />
                        ))}
                    </div>
                </div>
