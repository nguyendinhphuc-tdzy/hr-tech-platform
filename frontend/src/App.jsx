...
import AccountSettings from './views/AccountSettings'; // Đã import AccountSettings
...
const setupAxiosUser = (user) => {
    if (user && user.email) {
        // Gửi email user lên header để Backend biết user nào đang request
...
// ==========================================
// 1. MÀN HÌNH TRANG CHỦ (HomeView - FIXED DARK MODE)
// ==========================================
const HomeView = ({ onNavigate }) => {
  return (
    <div style={{
        height: '100vh', 
        backgroundColor: '#09121D', /* Ép cứng màu tối */
        backgroundImage: 'radial-gradient(circle at 50% 0%, #1a2c42 0%, #09121D 100%)',
        display: 'flex', flexDirection: 'column',
        color: '#FFFFFF', /* Ép cứng chữ trắng */
        overflowX: 'hidden',
        ...
        <div style={{flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', padding: '0 20px', position: 'relative', zIndex: 1}}>
            ...
            <button onClick={() => onNavigate('signup')} style={{
                padding: '16px 45px', fontSize: '16px', fontWeight: '700', borderRadius: '50px', 
...
// ==========================================
// 2. MÀN HÌNH XÁC THỰC (FIXED DARK MODE)
// ==========================================
...
// ==========================================
// 3. DASHBOARD LAYOUT & APP CONTROLLER (SỬ DỤNG BIẾN CSS CHO THEME)
// ==========================================
const DashboardLayout = ({ user, onLogout }) => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'account-settings': return <AccountSettings user={user} />; // Điều hướng đến AccountSettings
...
function App() {
    ...
    useEffect(() => {
        supabase.auth.getSession().then(({ data: { session } }) => {
            if (session) {
                ...
                setupAxiosUser(googleUser); // Cấu hình Axios ngay khi có session
                setView('dashboard');
            }
        });
    }, []);
    ...
    const handleLoginSuccess = (userData) => {
        ...
        setupAxiosUser(userData); // Cấu hình Axios khi login thường
        setView('dashboard');
    };
    ...
    const handleLogout = async () => {
        if(window.confirm("Bạn muốn đăng xuất?")) {
            ...
            setCurrentUser(null);
            setupAxiosUser(null); // Xóa cấu hình Axios
            setView('home'); 
        }
    };
